...
A Lewis structure isn't worked out from any formula the teacher types. The generator builds it only when the formula has one central atom (CH₄, H₂O, SF₆, SO₄²⁻), and takes molecules with more than one central atom (ethanol, HCN, N₂H₄) from a list stored by hand. Teachers can then change it to make a wrong structure for a "find the mistake" question: a bond's order, an atom's lone electrons or formal charge, the brackets and charge, or the central atom. The settings store the starting structure (a formula or listed structure) plus the list of changes, not the whole structure, so the address stays short (ADR 0001) and resetting to the correct structure just clears the changes.
