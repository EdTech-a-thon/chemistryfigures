...
import { CYLINDER_SIZES, INSTRUMENTS, formatReading, roundReading, volumeScale } from './scale'
...
export const volumeSettings = defineSettings(
  ...
  (s) => ({ ...s, reading: roundReading(volumeScale(s.instrument, s.size), s.reading), span: Math.round(s.span) }),
)
...
export const answerLine = (s: VolumeSettings) =>
  `Reading: ${formatReading(volumeScale(s.instrument, s.size), s.reading)} mL`
